...
public class Login : ControllerBase
{
	...
	public async Task<LoginResponse> Post(LoginRequest request)
	{
		...
		// 플레이어 데이터 로드
		(errorCode, var playerId) = await _gameDb.PlayerTable.SelectIdByAccountIdAsync(accountId);
		if(errorCode != ErrorCode.None)
		{
		...
		errorCode = await _memoryDb.AuthManager.RegisterUserAsync(accountId, authToken, playerId);
		if(errorCode != ErrorCode.None)
		{
